...
use better_cursor::{BetterRead, BetterWrite, CustomWrite, StructRead, StructWrite};
...
impl QuestStrings {
    pub fn read_string_from_offset<T: BetterRead>(reader: &mut T, pos: u32) -> Result<String> {
        reader.seek_start(pos as u64)?;
        reader.read_string_shift_jis()
    }

    pub fn from_reader<T: BetterRead>(
        reader: &mut T,
        pos: u32,
